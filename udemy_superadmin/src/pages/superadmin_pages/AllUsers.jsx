...
// Local, DNS-proof fallback (inline SVG data URI)
const SAFE_FALLBACK =
...
const AllUsers = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        // IMPORTANT: this endpoint returns ALL users across roles
        const res = await axios.get(`${globalBackendRoute}/api/all-users`);
        setUsers(res.data);
        setTotalCount(res.data.length);
      } catch (error) {
  ...
  const getImageUrl = (avatar, role) => {
    if (avatar && role) {
      const normalized = avatar.replace(/\\/g, "/").split("/").pop();
      // Serve from your own server to avoid mixed/CORS issues
      return `${globalBackendRoute}/uploads/${role}/${normalized}`;
    }
