...
const colorForKey = (key = "") => {
  const s = String(key);
  let sum = 0;
  for (let i = 0; i < s.length; i++) sum = (sum + s.charCodeAt(i)) % colorPalette.length;
  return colorPalette[sum];
};
const coerceNumber = (v, fallback = 0) => {
  const n = Number(v);
...
const SuperadminDashboard = () => {
    ...
    const fetchCounts = async () => {
      ...
      const pickCount = (data) => {
        ...
        if (typeof data === "object") {
          if (typeof data.total !== "undefined") return coerceNumber(data.total, 0);
          if (typeof data.count !== "undefined") return coerceNumber(data.count, 0);
          if (data.data) {
            if (typeof data.data.total !== "undefined") return coerceNumber(data.data.total, 0);
            if (typeof data.data.count !== "undefined") return coerceNumber(data.data.count, 0);
      ...
      try {
        const res = await axios.get(`${globalBackendRoute}/api/dashboard-counts`);
        const payload = res.data || {};
        const coerced = Object.fromEntries(
          Object.entries(payload).map(([k, v]) => [k, coerceNumber(v, 0)])
        );
        ...
        setCounts({ ...DEFAULT_COUNT_KEYS, activities: coerceNumber(aCount, 0) });
      }
    };
  ...
  useEffect(() => {
    const fetchInstructorCounts = async () => {
      try {
        const res = await axios.get(`${globalBackendRoute}/api/instructors/counts`);
        if (res.data?.success && res.data?.data) {
          const d = res.data.data;
          setInstructorCounts({
            pending: coerceNumber(d.pending, 0),
          ...
          setInstructorCounts({ pending: 0, approved: 0, rejected: 0, active: 0, inactive: 0 });
        }
      } catch (err) {
        console.error("Failed to fetch instructor counts", err);
        setInstructorCounts({ pending: 0, approved: 0, rejected: 0, active: 0, inactive: 0 });
  ...
  useEffect(() => {
    ...
    const fetchContactCounts = async () => {
      try {
        // Unread
        const unreadRes = await axios.get(`${globalBackendRoute}/api/messages/unread-count`);
        const unread =
          coerceNumber(unreadRes.data?.unreadCount, 0) ??
          coerceNumber(unreadRes.data?.count, 0) ??
          coerceNumber(unreadRes.data?.data?.unread, 0);
        ...
        let total = 0;
        try {
          const totalRes = await axios.get(`${globalBackendRoute}/api/messages/get-messages-count`);
          if (typeof totalRes.data === "object" && totalRes.data) {
            if (typeof totalRes.data.total !== "undefined")
              total = coerceNumber(totalRes.data.total, 0);
            else if (typeof totalRes.data.count !== "undefined")
              total = coerceNumber(totalRes.data.count, 0);
            else if (totalRes.data.data && typeof totalRes.data.data.total !== "undefined")
              total = coerceNumber(totalRes.data.data.total, 0);
            else total = 0;
          }
        } catch {
          try {
            const allRes = await axios.get(`${globalBackendRoute}/api/all-messages`);
            const arr = Array.isArray(allRes.data) ? allRes.data : allRes.data?.data || [];
            total = arr.length || 0;
          } catch {
            total = 0;
          }
    ...
    fetchContactCounts();
    timer = setInterval(fetchContactCounts, 30000);
    const onVis = () => document.visibilityState === "visible" && fetchContactCounts();
    document.addEventListener("visibilitychange", onVis);
    return () => {
      clearInterval(timer);
      document.removeEventListener("visibilitychange", onVis);
  ...
  const allCards = useMemo(() => {
    const ordered = [
      contactQuickCard, // shows unread dot
      ...baseCards, // includes "contacts" and "activities" etc.
      ...instructorBreakdownCards,
    ];
    const byKey = {};
    for (const c of ordered) if (c && !byKey[c.key]) byKey[c.key] = c;
  ...
  const filteredCards =
    search.trim() === ""
      ...
      : allCards.filter((card) => {
          const text = `${card.title} ${card.value}`.toLowerCase();
          const queryWords = search.toLowerCase().split(/\s+/).filter((word) => word && !stopwords.includes(word));
          return queryWords.some((word) => text.includes(word) || text.includes(word.replace(/s$/, "")));
        });
        ...
        <DashboardLayout
          left={
            <LeftSidebarNav
              navigate={navigate}
              items={[
                { label: "Account Settings", icon: <FaCog className="text-indigo-600" />, path: `/profile/${userId}` },
                { label: "Add Blog", icon: <FaPlus className="text-red-600" />, path: "/add-blog" },
                { label: "Add Category", icon: <FaPlus className="text-orange-400" />, path: "/add-category" },
                { label: "Add Sub Category", icon: <FaPlus className="text-orange-600" />, path: "/add-sub-category" },
                { label: "Add Degree", icon: <FaBoxOpen className="text-green-600" />, path: "/create-degree" },
                { label: "Add Semesters", icon: <FaPlus className="text-orange-600" />, path: "/create-semister" }, // route name kept to your existing
                { label: "Create Course", icon: <FaPlus className="text-green-400" />, path: "/create-course" },
                { label: "Create Exam", icon: <FaPlus className="text-green-600" />, path: "/create-exam" },
                { label: "Create Student", icon: <FaPlus className="text-green-600" />, path: "/student-register" },
                { label: "Create Quiz", icon: <FaPlus className="text-green-600" />, path: "/create-quiz" },
                { label: "Create Question", icon: <FaPlus className="text-fuchsia-600" />, path: "/create-question" },
                { label: "Create Notification", icon: <FaPlus className="text-purple-500" />, path: "/create-notification" },
                { label: "Create Activity", icon: <FaPlus className="text-purple-800" />, path: "/create-activity" },
              ]}
            />
          }
          right={
            ...
            >
              {filteredCards.map((card) => {
                const cardEl = (
                  <DashboardCard key={card.key} card={card} view={view} onClick={() => navigate(card.link)} />
                );

                // Red dot for unread contact messages
                if (card.key === "contact_messages" && coerceNumber(card.unread, 0) > 0) {
                  return (
                    <div key={card.key} className="relative" onClick={() => navigate(card.link)}>
                      {cardEl}
                      <span
                        title={`${card.unread} unread`}
                        className="absolute -top-1 -right-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-red-500 ring-2 ring-white"
