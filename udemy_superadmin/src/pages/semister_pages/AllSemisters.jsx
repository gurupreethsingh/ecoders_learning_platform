// // src/pages/semister_pages/AllSemisters.jsx
...
// export default function AllSemisters() {
...
//       return serverSlug;
//     const base = name || (semNumber ? `semester ${semNumber}` : "semister");
//     return String(base)
//       .toLowerCase()
//       .trim()
//       .replace(/[^a-z0-9\s-]/g, "")
...
//   // For semisters, show academicYear as a "tag" (so the UI matches)
...
//       } catch (err) {
//         if (!alive) return;
//         console.error("Error fetching semisters:", err);
//         setFetchError("Failed to load semisters. Please try again.");
//       } finally {
//         if (!alive) return;
//         setLoading(false);
//       }
...
//   // --- delete one semister ---
//   const deleteSemister = async (e, id, name) => {
//     // ensure clicking delete does not trigger the card link
//     e.preventDefault();
//     e.stopPropagation();

//     const ok = window.confirm(
//       `Delete semister "${name || "Untitled"}"? This action cannot be undone.`
//     );
//     if (!ok) return;

//     try {
//       // semister delete endpoint
//       const res = await axios.delete(
//         `${globalBackendRoute}/api/semisters/${id}`
//       );
...
//         }
//         setRefreshKey((k) => k + 1);
//         alert("Semister deleted successfully.");
//       } else {
//         throw new Error("Failed to delete semister.");
//       }
//     } catch (err) {
...
//           <h2 className="font-bold text-xl">All Semisters</h2>
//         </div>
...
//           <input
//             type="text"
//             placeholder="Search semisters by name, code, slug, academic year, description…"
//             value={searchTerm}
...
//             Showing {rows.length} of {meta.total} semisters
//           </p>
//           <FaThList
...
//       {/* Loading / Error */}
//       {loading && (
//         <p className="text-center text-gray-600 mt-6">Loading semisters…</p>
//       )}
//       {fetchError && !loading && (
...
//               const slug = makeSlug(d?.semister_name, d?.slug, d?.semNumber);
//               const path = `/single-semister/${slug}/${d?._id || d?.id}`;
...
//                     onClick={(e) => deleteSemister(e, d._id || d.id, title)}
//                   >
...
//                         {/* Optional summary-like block (mirrors Duration/Semesters) */}
//                         {view !== "list" &&
              ...
              return (
                <div key={semId} className="relative">
                  ...
                  <button
                    title="Delete semister"
                    className="absolute -top-2 -right-2 z-10 inline-flex items-center justify-center h-8 w-8 rounded-full bg-white border shadow hover:bg-red-50 text-red-600"
                    onClick={(e) => deleteSemister(e, semId, title)}
                  >
                    <FaTrashAlt className="h-4 w-4" />
                  </button>
