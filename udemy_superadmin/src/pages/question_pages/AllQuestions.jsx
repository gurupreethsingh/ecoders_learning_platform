// // src/pages/question_pages/AllQuestions.jsx
...
//   // lookups (maps for rendering fallback names)
...
//   // Reset pagination when search/pageSize/filters change
//   useEffect(() => setPage(1), [searchTerm, pageSize, filters]);

//   /* -------------------------------------------------
//    * 1) Load base degrees and static lookups (users)
//    * ------------------------------------------------- */
//   useEffect(() => {
//     let alive = true;
//     (async () => {
...
//             dMap[d._id || d.id] = d.name || d.title || "Untitled Degree";
//           });
//           setDegreeMap(dMap);
//         }
...
//   /* -------------------------------------------------
//    * 2) On degree change → load only its semisters
//    *    Expected: GET /api/semisters?degreeId=<id>
//    *    (Your backend already used ?degree=<id> elsewhere;
//    *     we support both keys to be safe.)
//    * ------------------------------------------------- */
//   useEffect(() => {
//     let alive = true;
...
//         const sList = Array.isArray(list) ? list : [];
//         setSemisterList(sList);
...
//   /* -------------------------------------------------
//    * 3) On semister change → load only courses for (degree, semister)
//    *    Expected: GET /api/list-courses?degreeId=<d>&semisterId=<s>
//    * ------------------------------------------------- */
//   useEffect(() => {
//     let alive = true;
...
//         const cMap = {};
//         cList.forEach((c) => {
//           cMap[c._id || c.id] = c.title || c.name || "Untitled Course";
//         });
//         setCourseMap(cMap);
//       } catch {
//         // keep empty
...
//   /* -------------------------------------------------
//    * 4) On course change → load quizzes (and exams)
//    *    Quizzes: GET /api/list-quizzes-by-course/:courseId
//    *    Exams:   GET /api/get-by-degree-semester-course/:d/:s/:c
...
//         // exams by degree, semister, course (if provided)
//         const examPromise =
...
//         // attach cascade filter params only if present
...
//     const active = q?.active ?? q?.isActive;
//     const status = q?.status || "draft";
//     const difficulty = q?.difficulty || q?.level || "—";
//     const qType = q?.type || q?.questionType || "—";
//     const marks =
//       Number.isFinite(Number(q?.marks)) || q?.marks === 0 ? q?.marks : "—";
//     const neg =
//       Number.isFinite(Number(q?.negativeMarks)) || q?.negativeMarks === 0
//         ? q?.negativeMarks
//         : null;
...
//   // Generic select with label (adds id in option text)
//   const FilterSelect = ({ label, value, onChange, options, getOption }) => (
...
//               const tags = toTags(q?.tags);
//               const optionsCount = Array.isArray(q?.options)
//                 ? q.options.length
//                 : q?.optionsCount || 0;
//               const correctCount = Array.isArray(q?.correctOptions)
...
//                             {Number.isFinite(Number(q?.marks)) || q?.marks === 0
//                               ? q?.marks
//                               : "—"}
//                           </p>
