...
// //   const [courseStructure, setCourseStructure] = useState([]);
// //   const [contentMap, setContentMap] = useState({});
// //   const [quizNameToObj, setQuizNameToObj] = useState({}); // NEW: quiz display name -> quiz object
...
// //         // prefill answer draft for MCQ as -1 (no selection yet)
// //         if (arr.length > 0) {
// //           const first = arr[0];
...
//   const [courseStructure, setCourseStructure] = useState([]);
//   const [contentMap, setContentMap] = useState({});
//   const [quizNameToObj, setQuizNameToObj] = useState({});
...
//   // NEW: cache questions per quiz (id -> array)
//   const [questionsCache, setQuestionsCache] = useState({}); // { [quizId]: Question[] }
//   // NEW: expanded quiz -> show nested list of questions
//   const [expandedQuizName, setExpandedQuizName] = useState(null);
//   // NEW: answered per quiz/question
//   const [answeredMap, setAnsweredMap] = useState({}); // { [quizId]: Set(questionId) }
...
//   // fetch and run quiz when the main panel needs it (selected quiz)
//   useEffect(() => {
//     // reset run state on topic change
//     setQuizQuestions([]);
//     setQuizLoading(false);
//     setQIndex(0);
//     setAnswerDraft(null);
...
//     const useCached = questionsCache[quizId];
//     if (useCached && Array.isArray(useCached) && useCached.length) {
//       setQuizQuestions(useCached);
//       const first = useCached[0];
//       if (first?.question_type === "mcq") setAnswerDraft(-1);
//       else if (first?.question_type === "true_false") setAnswerDraft(true);
//       else setAnswerDraft("");
//       return;
...
//         const arr = pickArray(res.data);
//         setQuizQuestions(arr);
//         setQuestionsCache((prev) => ({ ...prev, [quizId]: arr })); // cache
//         if (arr.length > 0) {
//           const first = arr[0];
//           if (first.question_type === "mcq") setAnswerDraft(-1);
//           else if (first.question_type === "true_false") setAnswerDraft(true);
...
//     // NEW: mark this question as answered in the nested sidebar
//     const quizObj = quizNameToObj[selectedTopic];
//     const quizId = quizObj?._id || quizObj?.id;
//     if (quizId && currentQuizQ?._id) {
//       setAnsweredMap((prev) => {
//         const prevSet = new Set(prev[quizId] || []);
//         prevSet.add(String(currentQuizQ._id));
...
//   /* =========================================================
//      Lazy fetch questions for a quiz (for nested list)
//   ========================================================= */
//   const ensureQuizQuestionsLoaded = async (quizName) => {
//     const qObj = quizNameToObj[quizName];
//     const quizId = qObj?._id || qObj?.id;
//     if (!quizId) return [];
//     if (questionsCache[quizId]) return questionsCache[quizId];
...
//       });
//       const arr = pickArray(res.data);
//       setQuestionsCache((prev) => ({ ...prev, [quizId]: arr }));
//       return arr;
//     } catch {
//       return [];
//     }
...
//     // expand/collapse the nested list under that quiz
//     const newName = expandedQuizName === quizName ? null : quizName;
//     setExpandedQuizName(newName);
//     if (newName) await ensureQuizQuestionsLoaded(quizName);
//   };

//   const jumpToQuestion = async (quizName, qId) => {
//     // make sure we're on that quiz in the main panel
//     handleTopicChange(quizName);
//     // ensure questions are loaded for accurate index
//     const list = await ensureQuizQuestionsLoaded(quizName);
//     const idx = list.findIndex((q) => String(q._id) === String(qId));
//     if (idx >= 0) {
...
//                               const quizObj = quizNameToObj[quizName];
//                               const quizId = quizObj?._id || quizObj?.id;

//                               // questions for this quiz from cache
//                               const qList = quizId
//                                 ? questionsCache[quizId] || []
//                                 : [];
//                               const answeredSet = quizId
//                                 ? answeredMap[quizId] || new Set()
//                                 : new Set();

//                               const isExpanded = expandedQuizName === quizName;
...
//                                       {quizName}
//                                     </button>
//                                     {/* show a check if *all* its questions answered (optional) */}
//                                     {qList.length > 0 &&
//                                     answeredSet.size >= qList.length ? (
//                                       <FaCheckCircle className="text-green-500 text-xs" />
//                                     ) : null}
//                                     <button
//                                       onClick={() =>
...
//                                       ) : (
//                                         qList.map((qItem, qi) => {
//                                           const done = answeredSet.has(
//                                             String(qItem._id)
//                                           );
//                                           // a short label for the question
//                                           const label =
//                                             qItem?.question_text?.slice(
//                                               0,
//                                               60
//                                             ) || `Question ${qi + 1}`;
...
//                                                   // highlight if this is the current question in main panel
//                                                   isQuizSelected &&
//                                                   selectedTopic === quizName &&
