...
// function stripBaseUrlToRelPath(filePathOrUrl, req) {
//   if (!filePathOrUrl) return "";

//   // If a full URL was ever saved/used, remove protocol+host
//   const base = `${req.protocol}://${req.get("host")}/`;
//   let rel = filePathOrUrl.startsWith(base)
//     ? filePathOrUrl.slice(base.length)
//     : filePathOrUrl;

//   // Remove any leading slash so path.join doesn't treat as absolute
//   rel = rel.replace(/^\/+/, "");

//   // Normalize backslashes -> forward slashes first, then let path.resolve handle OS specifics
//   rel = rel.replace(/\\/g, "/");
...
//     return true;
//   } catch (err) {
//     // Ignore "file not found"; rethrow others for logging
...
//     // Build absolute path: uploads/blogs/... relative to process CWD
//     // (Assumes you start Node from your backend root. If not, change to your known root)
//     const relStored = stripBaseUrlToRelPath(blog.featuredImage || "", req);

//     // Only attempt delete if it points inside uploads/blogs
//     // (defensive: prevents accidental deletion outside uploads)
//     if (relStored && relStored.startsWith("uploads/blogs")) {
//       const absPath = path.resolve(process.cwd(), relStored);

//       try {
//         await safeUnlink(absPath);
//       } catch (fileErr) {
//         console.error("Error deleting image:", fileErr);
//         // Don't block blog deletion because of a file system hiccup
//       }
//     }
...
const updateBlog = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      blog.publishedDate = blog.published ? blog.publishedDate || new Date() : null;
    }
