...
//       { instructions: { $regex: q.q, $options: "i" } },
...
//   // context (OPTIONAL)
//   if (q.context_degree)
//     filter["context.degrees"] = { $in: [toObjectId(q.context_degree)] };
//   if (q.context_semester)
//     filter["context.semesters"] = { $in: [toObjectId(q.context_semester)] };
//   if (q.context_course)
//     filter["context.courses"] = { $in: [toObjectId(q.context_course)] };
...
//     // collect submission file paths to delete from disk
...
function buildActivityFilter(q = {}) {
  ...
  // Prefer $text search when q is present (uses text index on title/instructions/tags)
  if (q.q) {
    // $text can co-exist with other fields (Mongo will AND them)
...
exports.create = asyncHandler(async (req, res) => {
  ...
  // defensive normalization if client ever sends tags as CSV
  const payload = { ...req.body };
