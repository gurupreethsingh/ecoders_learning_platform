...
// GET /api/users/by-role?role=instructor
const getUsersByRole = async (req, res) => {
  try {
    const { role } = req.query;
    if (!role) {
      return res.status(400).json({ message: "Missing role query param" });
    }
    const users = await User.find({ role }).select("_id name email role");
    return res.status(200).json(users);
  } catch (err) {
    console.error("getUsersByRole error:", err.message);
...
const getInstructors = async (req, res) => {
  try {
    ...
    console.error("getInstructors error:", err.message);
    return res.status(500).json({ message: "Server error" });
  }
};
...
const getAuthors = async (req, res) => {
  try {
    const users = await User.find({ role: "author" }).select(
      "_id name email role"
    );
    return res.status(200).json(users);
  } catch (err) {
    console.error("getAuthors error:", err.message);
    return res.status(500).json({ message: "Server error" });
  }
};
